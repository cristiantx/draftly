...
var ParagraphPlugin = class extends chunkOCI3S4CF_cjs.DraftlyPlugin {
  name = "paragraph";
  version = "1.0.0";
...
var HeadingPlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "heading";
  version = "1.0.0";
...
var InlinePlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "inline";
  version = "1.0.0";
...
var LinkPlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "link";
  version = "1.0.0";
...
var ListPlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "list";
  version = "1.0.0";
  decorationPriority = 20;
  requiredNodes = [
  ...
  decorateListMark(node, line, decorations, cursorOnMark) {
    const { from, to } = node;
    const parent = node.node.parent;
...
function pointerPosition(view, event) {
  const document2 = view.contentDOM.ownerDocument;
  const hit = document2.elementFromPoint(event.clientX, event.clientY);
...
var TablePlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
   ...
   */
  destroyedViews = /* @__PURE__ */ new WeakSet();
  ...
  handleDomKeydown(view, event) {
    if (event.target instanceof Element && event.target.closest("button") || event.defaultPrevented || event.isComposing || event.altKey || event.metaKey || event.ctrlKey) {
      return false;
    }
    let handled = false;
    if (event.key === "Tab") {
...
var HTML_NODE_NAMES = ["HTMLBlock", "HTMLTag", "Comment", "CommentBlock"];
...
var HTMLPlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "html";
  version = "1.0.0";
  decorationPriority = 30;
  requiredNodes = HTML_NODE_NAMES;
  constructor() {
    super();
  }
  /**
...
var ImagePlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "image";
  version = "1.0.0";
...
var CROSS_ICON = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`;
var CODE_FENCE = "```";
...
var CodeBlockHeaderWidget = class extends view.WidgetType {
  ...
  copyToClipboard(copyBtn) {
    const showResult = (className, icon, title) => {
    ...
    navigator.clipboard.writeText(this.codeContent).then(() => showResult("copied", CHECK_ICON, "Copied")).catch(() => showResult("copy-failed", CROSS_ICON, "Copy failed"));
  }
...
var CodePlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "code";
  version = "1.0.0";
  decorationPriority = 25;
...
${CODE_FENCE}`;
  ...
  decorateFenceMarkers(node, showFenceMarkers, decorations) {
    for (let child = node.firstChild; child; child = child.nextSibling) {
...
var QuotePlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "quote";
  version = "1.0.0";
  decorationPriority = 10;
...
var HRPlugin = class extends chunkOCI3S4CF_cjs.DecorationPlugin {
  name = "hr";
  version = "1.0.0";
  decorationPriority = 10;
...
function createEssentialPlugins() {
  return [
    new ParagraphPlugin(),
    new HeadingPlugin(),
    new InlinePlugin(),
...
//# sourceMappingURL=chunk-JLNUJ46A.cjs.map
//# sourceMappingURL=chunk-JLNUJ46A.cjs.map